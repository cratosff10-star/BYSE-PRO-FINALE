...
export function getSettings() {
  return db.prepare('SELECT * FROM reminder_settings WHERE id = 1').get() as any;
}

function renderTemplate(template: string, customer: any, purchase: any): string {
  const items = JSON.parse(purchase.items_json) as Array<{ name: string; qty: number }>;
...
export async function runReminderBatch(): Promise<{ sent: number; skipped: number; failed: number }> {
